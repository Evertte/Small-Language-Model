...
# -----------------------------------------------------------------------------
# default config values tuned for from-scratch training on mental_health_chat
# I/O
out_dir = 'out'
eval_interval = 2000
log_interval = 1
...
final_ckpt_name = 'final.pt'
init_from = 'scratch' # 'scratch' or 'resume' or 'gpt2*'
# wandb logging
wandb_log = False # disabled by default
wandb_project = 'mental-health-chat'
wandb_run_name = 'scratch-gpt' # 'run' + str(time.time())
# data
...
compile = False # on Colab T4, leave off by default for reliability
# -----------------------------------------------------------------------------
    ...
    if iter_num % eval_interval == 0 and master_process:
        ...
        if iter_num > 0:
            ...
            if always_save_checkpoint:
                print(f"saving latest checkpoint to {out_dir}/ckpt.pt")
                torch.save(checkpoint, os.path.join(out_dir, 'ckpt.pt'))
            if val_loss < best_val_loss:
                best_val_loss = val_loss
                checkpoint['best_val_loss'] = best_val_loss
                best_ckpt_path = os.path.join(out_dir, best_ckpt_name)
                print(f"saving best checkpoint to {best_ckpt_path} (val {best_val_loss:.4f})")
